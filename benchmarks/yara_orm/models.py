...
class ManyQuestionSchema(Schema):
    ...
    tags = flds.Method("get_tags", dump_only=True)

    def get_tags(self, obj):
        # We check whether a prefetch cache dictionary exists on the
        # model and whether it contains the 'tags' key.
        prefetch_cache = getattr(obj, "_prefetch", {})

        # If objects in the list under that key, pass them to TagSchema.
        if "tags" in prefetch_cache:
            return TagSchema(many=True).dump(prefetch_cache["tags"])
...
class SingleQuestionSchema(Schema):
    ...
    tags = flds.Method("get_tags", dump_only=True)
    user = flds.Nested(BaseUserSchema, dump_only=True)
    question_answers = flds.Method("get_question_answers", dump_only=True)

    def get_tags(self, obj):
        # we check whether a prefetch cache dictionary for keys.
        prefetch_cache = getattr(obj, "_prefetch", {})
    ...
    def get_question_answers(self, obj):
        ...
        # if objects in the list under that key, pass them to AnswerSchema
        if "question_answers" in prefetch_cache:
            return AnswerSchema(many=True, exclude=["question"]).dump(
                prefetch_cache["question_answers"]
            )
        # fallback
        return []
